...
public class RoleService extends DeleteService<Role> {
    ...
    public List<String> deleteMenu(RoleRequest request,List<Menu> menuAll,List<String> menuRecursionList){
        ...
        for(Menu parentMenu : menuAll){
            if(parentMenu.getMenuId().equals(request.getMenuPid())){
                menuRecursionList = this.deleteMenuParentByRecursion(menuAll,parentMenu,menuRecursionList);
            }
        }
        //根据菜单ID 找到所有子级,递归查询
        for(Menu menu : menuAll){
    ...
    public List<String> deleteMenuParentByRecursion(List<Menu> menuAll,Menu parentMenu,List<String> menuRecursionList){
        int count = 0;
        Menu resultMenu = null;
        for(Menu menu : menuAll){
            if(menu.getMenuId().equals(parentMenu.getMenuPid())){
                if(!StringUtil.isEmpty(menu.getMenuIsNull())){
                    count ++;
                }else {
                    resultMenu = menu;
        ...
        if(count == 0){
            menuRecursionList.add(resultMenu.getMenuId());
            if(!StringUtil.isEmpty(resultMenu.getMenuPid())){
                menuRecursionList = deleteMenuParentByRecursion(menuAll,resultMenu,menuRecursionList);
            }
        }
        return menuRecursionList;
    }
